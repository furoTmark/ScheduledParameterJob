...
namespace ScheduledParameterJob
{
	public class DatabaseJobAdapter : PageAdapter
	{
		...
		private ScheduledPlugInWithParametersAttribute Attribute
		{
			get
			{
				if (_attribute == null && !string.IsNullOrEmpty(PluginId))
				{
		...
		{
			if (PersistedValues.TryGetValue(controlDto.Id, out var persistedValue))
			{
				Definitions.SetValue(controlDto.Control, persistedValue);
			}
		}
		...
		private void AddStylesheet()
		{
			var cssPath = Page
				.ClientScript
				.GetWebResourceUrl(typeof(DatabaseJobAdapter), "ScheduledParameterJob.Style.JobParameters.css");
